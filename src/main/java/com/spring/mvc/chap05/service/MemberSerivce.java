...
public class MemberSerivce {
    ...
    public boolean join(SignUpRequestDTO dto, String savePath) {
        ...
        dto.setLoginMethode(LoginMethode.COMMON);
...
//        member.setProfileImage(savePath);
    ...
    public LoginResult authenticate(
        ...
        //자동 로그인 처리
        if (dto.isAutoLogin()){
//            1. 자동 로그인 쿠키 생성 - 쿠키 안에 절대 중복되지 않는 값 (브라우저 세션 아이디) 저장 - JSESSIONS
...
//          4. DB에도 쿠키에 관련된 값(랜덤 세션키, 만료시간)들을 갱신
    ...
    public void maintainLoginState(HttpSession session, String account) {
        ...
        LoginUserResponseDTO dto = LoginUserResponseDTO.builder()
                ...
                .loginMethod(member.getLoginMethode().toString())
                .build();
...
//        2. 쿠키를 삭제 한다. -> 쿠키 수명을 0초로 설정하여 다시 클라이언트에 전송
        if (c != null){
            c.setMaxAge(0);
            c.setPath("/");
