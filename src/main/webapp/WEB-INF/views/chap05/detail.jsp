...
<div id="wrap" class="form-container">
    ...
    <script>
        ...
        function makePageButtonClickEvent() {
            const $pageUl = document.querySelector('.pagination');
            $pageUl.onclick = e => {
                //이벤트 타겟이 a링크가 아닌경우 href속성을 못가져올 수 있으니 타겟 제한
                if (!e.target.matches('.page-item a')) return;

                console.log(e.target.getAttribute('href'));
                e.preventDefault(); //href 링크 이동 기능 중단
        ...
        function makeReplyPostClickEvent() {
            ...
            $addBtn.onclick = e => {
                ...
                if (textVal.trim() === '') {
                    alert('댓글 내용은 필수값입니다.');
                    return;
                } else if (writerVal.trim() === '') {
                    alert('댓글 작성자는 필수값입니다.!!');
                    return
                } else if (writerVal.trim().length < 2 || writerVal.trim > 8) {
                    alert('댓글 작성자는 2~8글자 사이로 작성!')
                    return
                }
                ...
                const requestInfo = {
                    ...
                    body: JSON.stringify(payload) //JSON으로 변환하는 코드
                }
                //서버에 POST요청 보내기
                fetch(URL, requestInfo)
                    .then(res => {
                        if (res.status === 200) {
                            alert('댓글이 정상 등록되었습니단');
                            return res.json();
                        } else {
                            alert('댓글 등록에 실패했습니단!');
                            return res.text();
