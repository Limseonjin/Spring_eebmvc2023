...
class ScoreRepositoryImplTest {
    ...
    // 단언(Assertion) 기법 : ~해야한다. ~이다. (O) / ~할 것이다. ~일 것 같다(X)
    //GWT패턴 : Given, When, Then 패턴

    @Test
    @DisplayName("저장소에서 findAll 메서드를 호출하면 "+
            "리스트가 반환되고, 해당 리스트에는 성적정보가 3개 들어있어야한다.")
    void findAllTest(){
        //GWT패턴
    ...
    @Test
    @DisplayName("저장소에서 findOne을 호출하여 학번이 2인 학생을 조회하면 그 학생의 국어점수가 33점이고 이름은 춘식이어야 한다")
    void findOneTest(){
        //given
        int stuNum = 2;
        //when
    ...
    @DisplayName("새로운 성적정보를 save를 통해 추가하면" +
            "목록의 개수가 4개여야 한다.")
    void saveTest() {
        Score newS = new Score("랑이", 100, 100, 55, 4, 0, 0.0, Grade.F);

        boolean save = repository.save(newS);
