...
*/
    ...
    @Test
    @DisplayName("77번 게시물의 댓글 목록을 조회했을 때 결과리스트의 사이즈는 14이어야 한다")
    void findAllTest() {
        //given
        long boardNo = 77L;
        //when
        ...
        //then
        assertEquals(14, replyList.size());
        assertEquals("잼민이 25", replyList.get(0).getReplyWriter());
    }


    @Test
    @DisplayName("77번 게시물의 25번 댓글을 삭제하면 25번 댓글은 조회되지 않을 것이고" +
            " 77번을 전체조회하면 리스트의 사이즈는 13이어야 한다.")
    void deleteTest() {
        //given
        long boardNo = 77;
        long replyNo = 25;
        ...
        //then
        assertNull(reply);
        assertEquals(13, replyMapper.findAll(boardNo,new Page()).size());
    }
